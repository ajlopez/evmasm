...
function LabelReferenceExpression(name) {
	...
	this.resolve = function (labels) {
		if (labels[name] != null && labels[name].offset != null)
			offset = labels[name].offset;
	}
}
...
function AuxdataExpression(value) {
	this.value = function () { return value; };
...
function CodeSegment(label, exprs) {
	...
	function hasJumpLabel() {
		...
		if (exprs.length[0] instanceof AssemblyExpression)
			return false;
...
function Assembly(segments) {
	...
	function calculateLabelData() {
		...
		for (var k = 0; k < segments.length; k++) {
			...
			var codesize = segments[k].codesize();
	...
	this.parseSegment = function () {
		...
		if (token && token.type === TokenType.Label) {
			if (token.value === 'auxdata') {
				var auxdata = parseInteger();
				var auxdataexpr = new AuxdataExpression(auxdata);
